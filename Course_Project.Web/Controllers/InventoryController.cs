    ...
    public class InventoryController : Controller
    {
        ...
        public async Task<IActionResult> Details(string id)
        {
            ...
            if (response != null && (await Check(response)||await _inventoryService.CanEdit(response, User.Identity.Name))) 
                                              return View(new InventoryEditViewModel() { inventory = response,Photo= await _cloudService.GetPhotoAsync(response.PhotoLink),
                                              Ids = Generator.GenerateExample(response.CustomSetOfIds),
                                              Users = await _userService.GetAllUsersAsync(),UserAccess=await _inventoryService.GetAccess(response.PublicId),
        ...
        public async Task<IActionResult>Save(SaveInventory saveInventory)
        {
            ...
            await _tagService.UpdateTagsForInventoryAsync(inventory.PublicId.ToString(), System.Text.Json.JsonSerializer.Deserialize<List<string>>(saveInventory.Tags) ?? new List<string>());
